...
inline int CalcMandelbrot(const float xPos, const float yPos, const int crunch)
{
    float y = yPos;
    float x = xPos;
    float yy = y * y;
...
inline int CalcMandelbrotDS(const dfloat xPos, const dfloat yPos, const int crunch)
{
    ...
    while (--i && (sum[0] < 4.0f)) {
        dsmul(y, x, y);     // y = x * y * 2.0f + yPos;
        dsadd(y, y, y);
        dsadd(y, y, yPos);

        dssub(x, xx, yy);   //  x = xx - yy + xPos;
        dsadd(x, x, xPos);
...
void RunMandelbrotGold0(uchar4 *dst, const int imageW, const int imageH, const int crunch, const float xOff, const float yOff, const float scale, const uchar4 colors, const int frame, const int animationFrame)
{
...
void RunMandelbrotDSGold0(uchar4 *dst, const int imageW, const int imageH, const int crunch, const double xOff, const double yOff, const double scale, const uchar4 colors, const int frame, const int animationFrame)
{
    for (int iy = 0; iy < imageH; iy++)
        for (int ix = 0; ix < imageW; ix++) {
		    // Calculate the location
            ...
            // Calculate the Mandelbrot index for the current location
            int m = CalcMandelbrotDS(dsx, dsy, crunch);
            m = m > 0 ? crunch - m : 0;
...
void RunMandelbrotGold1(uchar4 *dst, const int imageW, const int imageH, const int crunch, const float xOff, const float yOff, const float scale, const uchar4 colors, const int frame, const int animationFrame)
{
    for (int iy = 0; iy < imageH; iy++)
        for (int ix = 0; ix < imageW; ix++) {
...
void RunMandelbrotDSGold1(uchar4 *dst, const int imageW, const int imageH, const int crunch, const double xOff, const double yOff, const double scale, const uchar4 colors, const int frame, const int animationFrame)
{
    for (int iy = 0; iy < imageH; iy++)
        for (int ix = 0; ix < imageW; ix++) {
		    // Get the current pixel color
